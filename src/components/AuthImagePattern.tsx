...
import type { AuthImagePatternProps } from "../types/authImagePattern";

const CodeBackground = ({ title, subtitle }: AuthImagePatternProps) => {
...
function reverseList(head) {
  ...
  const fileNames = ["two-sum.js", "reverse-list.js", "valid-parentheses.js"];
          ...
          {codeSnippets.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                activeIndex === index ? "w-8 bg-primary" : "w-2 bg-slate-500"
              }`}
